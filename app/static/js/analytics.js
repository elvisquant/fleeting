/** 
 * analytics.js - Fully Featured Fleet Analytics
 */
...
// Element Selector (Dashboard Model Style)
function getAnEl(id) {
...
async function initAnalytics() {
    console.log("Analytics: Full Init");
...
async function loadAnalyticsData() {
    try {
        ...
        updateAnKPIs(data);
        updateDistributionText(data); // Fixed logic for percentages
        renderAnCharts(data);
...
function getAnRange(p) {
    ...
    else start.setFullYear(today.getFullYear() - 1); // default 12m
...
function renderAnCharts(data) {
    ...
    if (barCtx) {
        ...
        monthlyChart = new Chart(barCtx, {
            ...
            data: {
                ...
                datasets: [
                    { label: 'Fuel', data: sorted.map(i => i.fuel_cost), backgroundColor: '#ef4444' },
                    { label: 'Repair', data: sorted.map(i => i.reparation_cost), backgroundColor: '#f59e0b' },
                    { label: 'Maint', data: sorted.map(i => i.maintenance_cost), backgroundColor: '#3b82f6' }
                ]
            },
            options: {
    ...
    if (pieCtx) {
        ...
        distributionChart = new Chart(pieCtx, {
            type: 'doughnut',
            data: {
                labels: ['Fuel', 'Repair', 'Maint', 'Purchases'],
                datasets: [{
...
function updateQuickStats(data) {
    ...
    if (high) {
        const items = [
            { n: 'Fuel', v: data.total_fuel_cost }, { n: 'Repair', v: data.total_reparation_cost },
            { n: 'Maint', v: data.total_maintenance_cost }, { n: 'Purch', v: data.total_vehicle_purchase_cost }
        ];
...
// 4. Report Generation (Full Restored)
