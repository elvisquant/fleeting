...
(function() {
    ...
    function attachDashboardListeners() {
        ...
        const applyDateBtn = getDashEl('applyDateRange');
        if (applyDateBtn) {
            applyDateBtn.addEventListener('click', applyCustomDateRange);
        }
        
        // Refresh button
        const refreshBtn = getDashEl('refreshDashboard');
        if (refreshBtn) {
    ...
    async function loadDashboardData() {
        try {
            ...
            loadVehicleStatusChart(vehicleStatus);
            loadExpenseChart(expenseData);
    ...
    function formatBIF(amount) {
        // Use window.APP_LOCALE or default to 'en-US'
        const locale = window.APP_LOCALE || 'en-US';
        return `BIF ${(amount || 0).toLocaleString(locale, {
    ...
    function loadMonthlyChart(expenseData) {
        ...
        if (expenseData && expenseData.monthly_breakdown && expenseData.monthly_breakdown.length > 0) {
            ...
            labels = sortedBreakdown.map(item => {
                if (item.month_year) {
                    return new Date(item.month_year).toLocaleDateString(window.APP_LOCALE || 'en-US', { month: 'short', year: '2-digit' });
                }
                return 'Unknown';
            });
    ...
    function loadVehicleStatusChart(data) {
    ...
    function loadExpenseChart(expenseData) {
    ...
    function refreshDashboard() {
        ...
        refreshButtons.forEach(btn => {
            if(btn.innerText && btn.innerText.includes(window.t('btn_refresh'))) {
            ...
            alertsContainer.innerHTML = `
                <div class="flex items-center justify-center p-8">
                    <div class="text-center">
                        ...
                        <button onclick="window.dashboardModule.refreshDashboard()" class="mt-2 text-blue-400 hover:text-blue-300 text-xs flex items-center gap-1 mx-auto">
    ...
    window.dashboardModule = {
        ...
        destroy: function() {
            ...
            const applyDateBtn = getDashEl('applyDateRange');
            if (applyDateBtn) applyDateBtn.removeEventListener('click', applyCustomDateRange);
            
            const refreshBtn = getDashEl('refreshDashboard');
            if (refreshBtn) refreshBtn.removeEventListener('click', refreshDashboard);
    ...
    // Auto-initialize if dashboard is loaded directly
