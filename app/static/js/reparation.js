...
window.reqRepVerify = function(id) {
    repActionType = 'verify'; repActionId = id;
...
window.reqRepDelete = function(id) {
    repActionType = 'delete'; repActionId = id;
    showRepConfirmModal("Delete Reparation?", "This cannot be undone.", "trash-2", "bg-red-600");
}

async function executeRepConfirmAction() {
    if(!repActionId) return;
    const btn = document.getElementById('btnRepConfirmAction');
    btn.disabled = true; btn.innerText = "Processing...";

    try {
        let result;
        if (repActionType === 'delete') result = await window.fetchWithAuth(`/reparation/${repActionId}`, 'DELETE');
        if (repActionType === 'verify') result = await window.fetchWithAuth(`/reparation/${repActionId}`, 'PUT', { is_verified: true });

        window.closeModal('repConfirmModal');
        if (result !== null || repActionType === 'delete') await loadRepData();
        else alert("Action failed.");
    } catch(e) {
        window.closeModal('repConfirmModal');
        alert("Error: " + e.message);
    }
    btn.disabled = false; btn.innerText = "Confirm"; repActionId = null;
}
...
window.saveReparation = async function() {
    ...
    if(!payload.panne_id || !payload.garage_id || isNaN(payload.cost)) { alert("Please fill all required fields."); return; }
...
function showRepConfirmModal(t, m, i, c) {
    ...
    document.getElementById('repConfirmModal').classList.remove('hidden');
    if(window.lucide) window.lucide.createIcons();
}
