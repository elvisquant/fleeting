...
(function() {
    ...
    /**
     * ==============================================================================
     * FLEETDASH REQUESTS MODULE (Multi-Language)
     * Handles vehicle requests, approval workflow, and resource assignments.
     * ==============================================================================
     */
    ...
    function attachEventListeners() {
        // Table filters
        const search = getEl('reqSearch');
        const filter = getEl('reqStatusFilter');
        const addBtn = getEl('btnAddRequest');
        const saveBtn = getEl('btnSaveReq');
        const assignBtn = getEl('btnExecuteAssign');
        const approveBtn = getEl('btnExecuteApproval');
        
        if (search) search.addEventListener('input', renderTable);
        if (filter) filter.addEventListener('change', renderTable);
        if (addBtn) addBtn.addEventListener('click', openAddModal);
        if (saveBtn) saveBtn.addEventListener('click', saveRequest);
        if (assignBtn) assignBtn.addEventListener('click', executeAssignment);
        if (approveBtn) approveBtn.addEventListener('click', executeApprovalDecision);
    ...
    function renderTable() {
        ...
        tbody.innerHTML = filtered.map(r => {
            ...
            let statusHtml = '';
            if (r.status === 'denied') {
                statusHtml = `<span class="px-2 py-1 rounded text-[10px] uppercase font-bold bg-red-500/10 text-red-400 border border-red-500/20">
                    ${window.t('status_denied')}
                </span>`;
            } else {
                let step = 0;
            ...
            return `
                <tr class="${rowClass}">
                    ...
                    <td class="p-4 text-right">
                        <button onclick="window.requestsModule.openViewModal(${r.id})" 
                            class="p-1.5 bg-slate-800 text-blue-400 hover:bg-blue-600 hover:text-white rounded-md transition" 
                            title="${isLocked ? window.t('view') : window.t('edit')}">
    ...
    function openAddModal() {
        const modal = getEl('addRequestModal');
        if (!modal) return;
        ...
        const depEl = getEl('reqDeparture');
        const retEl = getEl('reqReturn');
    ...
    async function saveRequest() {
        const btn = getEl('btnSaveReq');
        if (!btn) return;

        const dest = getEl('reqDestination')?.value.trim();
        const dep = getEl('reqDeparture')?.value;
        const ret = getEl('reqReturn')?.value;
        const desc = getEl('reqDesc')?.value.trim();
        const passStr = getEl('reqPassengers')?.value;
        ...
        try {
            ...
            if (result && !result.detail) {
                ...
                showAlert(window.t('title_success'), window.t('req_submitted'), true);
            } else {
            ...
            showAlert(window.t('title_error'), e.message || window.t('msg_connection_fail'), false);
        }
    ...
    function openViewModal(id) {
        ...
        const viewContent = getEl('viewRequestContent');
        const footer = getEl('approvalActionsFooter');
        const modal = getEl('viewRequestModal');
        ...
        const requester = r.requester ? r.requester.full_name : "Unknown";
        const service = r.requester && r.requester.service ? r.requester.service.service_name : "-";
        const passengers = r.passengers ? (Array.isArray(r.passengers) ? r.passengers.join(", ") : r.passengers) : window.t('none');
        ...
        let rejectionHtml = r.status === 'denied' && r.rejection_reason ? `
            <div class="col-span-2 mt-2 p-3 bg-red-500/10 border border-red-500/20 rounded-lg">
                <span class="text-red-400 text-xs uppercase font-bold block mb-1">${window.t('status_denied')} - ${window.t('reason')}</span>
        ...
        let buttons = `<button onclick="window.requestsModule.closeModal('viewRequestModal')" 
            class="px-4 py-2 bg-slate-700 hover:bg-slate-600 text-white text-sm font-medium rounded-lg">
            ${window.t('btn_close')}
        </button>`;
        ...
        if (!isLocked) {
            ...
            if (['charoi', 'admin', 'superadmin'].includes(reqUserRole) && 
                ...
                const assignBtn = `
                    <button onclick="window.requestsModule.openAssignmentModal(${r.id})" 
                        class="px-4 py-2 bg-blue-600 hover:bg-blue-500 text-white text-sm font-medium rounded-lg flex items-center gap-2 mr-2">
    ...
    function getApproveRejectButtons(id, label) {
        return `
            <button onclick="window.requestsModule.closeModal('viewRequestModal')" 
                class="px-3 py-2 text-slate-400 hover:text-white text-sm font-medium">
                ${window.t('btn_cancel')}
            </button>
            <button onclick="window.requestsModule.openApprovalModal(${id}, 'reject')" 
                class="px-4 py-2 bg-red-600 hover:bg-red-500 text-white text-sm font-medium rounded-lg flex items-center gap-2">
                <i data-lucide="x-circle" class="w-4 h-4"></i> ${window.t('btn_deny')}
            </button>
            <button onclick="window.requestsModule.openApprovalModal(${id}, 'approve')" 
                class="px-4 py-2 bg-green-600 hover:bg-green-500 text-white text-sm font-medium rounded-lg flex items-center gap-2">
                <i data-lucide="check-circle" class="w-4 h-4"></i> ${window.t('btn_approve')}
            </button>
        `;
    }
    ...
    function openApprovalModal(id, type) {
        ...
        const commentEl = getEl('approvalComment');
        const titleEl = getEl('approvalTitle');
        const btn = getEl('btnExecuteApproval');
        const iconDiv = getEl('approvalIcon');
    ...
    async function executeApprovalDecision() {
        ...
        try {
            ...
            if (result && !result.detail) {
                await loadRequestsData();
                showAlert(window.t('title_success'), window.t('title_success'), true);
            } else {
                const msg = result?.detail ? (typeof result.detail === 'object' ? JSON.stringify(result.detail) : result.detail) : window.t('msg_operation_failed');
    ...
    function openAssignmentModal(requestId) {
        ...
        const modal = getEl('assignmentModal');
        const vSelect = getEl('assignVehicle');
        const dSelect = getEl('assignDriver');
        const idInput = getEl('assignReqId');
    ...
    async function executeAssignment() {
        const btn = getEl('btnExecuteAssign');
        const reqId = getEl('assignReqId')?.value;
        const vId = getEl('assignVehicle')?.value;
        const dId = getEl('assignDriver')?.value;
        ...
        try {
            ...
            if (result && !result.detail) {
                ...
                await loadRequestsData();
                showAlert(window.t('title_success'), window.t('msg_assigned'), true);
            } else {
                const msg = result?.detail ? (typeof result.detail === 'object' ? JSON.stringify(result.detail) : result.detail) : window.t('msg_operation_failed');
                showAlert(window.t('title_error'), msg, false);
            }
    ...
    function showAlert(title, message, isSuccess) {
        const container = getEl('toast-container') || getEl('reqAlertModal');
        if (!container) {
            console.log(`${title}: ${message}`);
            return;
        }

        // Check if using toast container (like reparation.js)
        if (container.id === 'toast-container') {
            ...
            // Fallback to existing alert modal
            const titleEl = getEl('reqAlertTitle');
            const messageEl = getEl('reqAlertMessage');
            const iconEl = getEl('reqAlertIcon');
    ...
    window.requestsModule = {
        init: initRequests,
        destroy: function() {
            console.log("Requests module cleanup");
        },
        refresh: loadRequestsData,
    ...
    if (document.readyState === 'complete' && window.location.hash === '#requests') {
        setTimeout(initRequests, 100);
    }
